...
def _convert_to_example(filename, image_data, labels, labels_text, bboxes, shape,
                        difficult, truncated):
...
def run(voc_root, split, output_dir, shuffling=True):
    ...
    split_file_path = os.path.join(voc_root,'ImageSets/Main/%s.txt'%split)
    ...
    i = 0
    tfrecord_writer = tf.python_io.TFRecordWriter(os.path.join(output_dir, split + ".tfrecords"))
    while i < len(filenames):

        sys.stdout.write('\r>> Converting image %d/%d' % (i+1, len(filenames)))
        sys.stdout.flush()
        filename = filenames[i].strip()
        _add_to_tfrecord(voc_root, filename, tfrecord_writer)
        i += 1

    tfrecord_writer.close()
    print('\nFinished converting the Pascal VOC {} dataset!'.format(split))

if __name__ == '__main__':
    voc_path = sys.argv[1]
    tf_path = sys.argv[2]

    run(voc_path, "trainval", tf_path)
    run(voc_path, "test", tf_path)
